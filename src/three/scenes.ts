...
const PORTRAIT_OVERRIDE: Partial<Record<SceneKey, { camera: SceneState['camera']; target: SceneState['target'] }>> = {
  ...
  // interval(세로): 띠는 대각선으로 놓여 있다(이른 출발일일수록 x가 크고 z가 작고, 늦을수록 반대 —
  // 방향 벡터 약 (-0.87, 0, 0.49)). 가로 화면 카메라([-12,5,12] → [0,0.5,0])는 이 대각선을 거의 정면으로
  // 바라봐 띠가 짧게 겹쳐 보인다(원근 단축).
  // 1차 시도: 띠에 정확히 수직인 방향(+z, +x 쪽 정면)에서 봐 화면 가로 전체(약 90%)에 펼쳤다.
  // 수학적으로는 화면 안에 다 들어왔지만(실측 스크린샷 확인), 실제 출발일 간격이 고르지 않다는 걸
  // 놓쳤다 — 가까운 출발일(D+3~40대)은 지형의 실측 날짜가 매일이라 넓게 퍼지고, 먼 출발일(D+60~90대,
  // 주 1회 수집)은 두 실측 날짜 사이를 보간해 채우다 보니 z 0.02~0.06 간격으로 촘촘히 뭉친다.
  // additive 블렌딩(shaders.ts)에서 촘촘한 쪽은 겹쳐 밝게 보이지만, 넓게 퍼진 쪽은 점 하나하나가
  // 흐려 지형 잡음에 묻혀 버렸다(세로 화면 캡처로 확인, 1차 시도는 오른쪽 끝만 밝고 나머지는 거의 안 보임).
  // 2차 시도(그대로 수직, 거리만 줄이고 비대칭으로 기울임)도 같은 문제였다 — 세로 화면 폭 제약상
  // "화면을 꽉 채우게 펼치기"와 "점을 키우기"를 동시에 만족하는 여유가 거의 없었다.
  // 3차: 띠 방향 쪽으로 카메라를 일부 돌려(순수 수직이 아니라 띠 방향 성분을 섞어) 원근 단축을 약간
  // 허용하고 거리를 줄였다. 그 결과 점들이 겹치며 밝은 덩어리를 이루면서도, 확대해 보면 개별 세로
  // 줄무늬가 뚜렷이 구분된다(가로 화면의 완전한 단축과 1차 시도의 완전한 펼침 사이 절충점).
  // 목표점 y를 낮춰(-1.5) 띠가 화면 위쪽 약 40~55%에 오게 해 아래 글 카드와 겹치지 않게 했다.
  interval: { camera: [16.397, 3.5, 4.583], target: [3, -1.5, 5.3] },
};
